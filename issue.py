...
class Issue(Document):
	...
	def on_update(self):
		...
		if self.items[0]:
			...
			for item in self.items:
				if not item.material_request_raised==1:
					material_request_items.append(item)
					item.material_request_raised=1
				if material_request_items:
					new_request=frappe.new_doc("Material Request")
					new_request.issue_type=self.name
					new_request.client=self.customer
					new_request.material_request_type="Material Issue"
					for material_item in material_request_items:
						data={}
						data['item_code']=material_item.item_code
						data['item_name']=material_item.item_name
						data['qty']=material_item.qty
						data['uom']=material_item.uom
						data['schedule_date']=material_item.schedule_date
						data['warehouse']=material_item.warehouse
					new_request.append("items",data)
					new_request.save()
	...
	def get_check_parameters(self):
		self.set('check_parameters', [])
		#parameters = get_template_details(self.quality_inspection_template)
		parameters = frappe.get_all('Check Parameters Table', fields=["description_of_work","serial_no1","serial_no2","serial_no3"],
				filters={'parenttype': 'Check Parameters List', 'parent': self._check_parameters1}, order_by="idx")
		for d in parameters:
			child = self.append('check_parameters', {})
			child.description_of_work=d.description_of_work
			child.serial_no1=d.serial_no1
			child.serial_no2=d.serial_no2
			child.serial_no3=d.serial_no3
		self.get_observations_table()
	def get_observations_table(self):
		self.set('observation',[])
		parameters=frappe.get_all('Observations in Issue',fields=["observations","first_reading","second_reading","third_reading"],
				filters={'parenttype':'Check Parameters List','parent':self._check_parameters1},order_by="idx")
		for d in parameters:
			child=self.append('observation',{})
			child.observations=d.observations
			child.first_reading=d.first_reading
			child.second_reading=d.second_reading
			child.third_reading=d.third_reading
		self.get_checklist_table()
	def get_checklist_table(self):
		self.set("checks",[])
		parameters=frappe.get_all('Checklist in Issue',fields=["checks","serial_1","serial_2","serial_3"],
				filters={'parenttype':'Check Parameters List','parent':self._check_parameters1},order_by="idx")
		for d in parameters:
			child=self.append('checks',{})
			child.checks=d.checks
			child.serial_1=d.serial_1
			child.serial_2=d.serial_2
			child.serial_3=d.serial_3
