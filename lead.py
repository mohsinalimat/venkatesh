...
def facebook_lead(**kwargs):
	frappe.log_error(kwargs, "Facebook log error")
	challenge = kwargs["hub.challenge"]
	token = kwargs["hub.verify_token"]
	if token == "abc123" and kwargs['hub.mode']=='subscribe':
		return Response(challenge)
	elif kwargs.get('object')=='page':
		access_token='''EAAHGWisDFO0BAGppvmZBZC246noUKKOCYBxgM7WFVREfyNsKdQFkdvpJbwhpKXqHJcecbpdhJQ7jZCeUvf4GBmTekbk5He7vt5hGtOpiiOQoZBZAgQ5CbIsjZB68bnYozCTQI0ODZALe5Oy7BaZApEqNLcN2X$'''
		GRAPH_API_ENDPOINT = 'https://graph.facebook.com/v9.0'
		for page in body.get('entry'):
			for change in page.get('changes'):
				...
				leadgen_uri = GRAPH_API_ENDPOINT + '/' + leadgen_id +f'?access_token={access_token}'
				response=json.loads(requests.request("GET",leadgen_uri).text)
				frappe.log_error(response,"Facebook Response")

@frappe.whitelist(allow_guest=True)
def call(phone,agent_id):
    url=f"https://112.196.108.76:8475/CrmDial?exeUserName={agent_id}@jeenasikho&phoneNumber={phone}&skill=TESTSKL&listId=1"
    response = requests.request("GET", url,verify=False)
    frappe.msgprint(response.text.encode('utf8'))

@frappe.whitelist(allow_guest=True)
def end_call(agent_id,disposition,time=None):
   if disposition=="HANGUP":
   	url=f"https://112.196.108.76:8475/CrmHangup?exeUserName={agent_id}@jeenasikho&disposition={disposition}:HU"
   else:
           #frappe.msgprint(str(time))
       url=f"https://112.196.108.76:8475/CrmHangup?exeUserName={agent_id}@jeenasikho&disposition={disposition}:CB &callbackDate={time} "
   #frappe.msgprint(str(agent_id))
   response=requests.request("GET",url,verify=False)
   frappe.msgprint(response.text.encode('utf-8'))
