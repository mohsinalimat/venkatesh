...
#			tax.total = flt(self.doc.total_invoice_amount)
	...
	def get_current_tax_amount(self, item, tax, item_tax_map):
		...
		if tax.charge_type == "Actual":
			...
			if self.doc.doctype == "Purchase Invoice" and not self.doc.is_return and self.doc.total_invoice_amount:
				current_tax_amount = (tax_rate / 100.0) * self.doc.total_invoice_amount
#			elif self.doc.doctype == "Purchase Invoice" and not self.doc.is_return and self.doc.total:
#				current_tax_amount = (tax_rate / 100.0) * self.doc.total
			else:
