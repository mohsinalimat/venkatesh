...
erpnext.accounts.PurchaseInvoice = erpnext.buying.BuyingController.extend({
			...
			if(doc.outstanding_amount >= 0 || Math.abs(flt(doc.outstanding_amount)) < flt(doc.grand_total)) {
			 ...
			 if(doc.outstanding_amount >= 0 || Math.abs(flt(doc.outstanding_amount)) < flt(doc.grand_total)) {
				if (doc.total_rejected > 0 && !doc.rejected_qty){
	                                cur_frm.add_custom_button(__('Return / Debit Note for Rejected Qty'),
			 ...
			 if(doc.outstanding_amount >= 0 || Math.abs(flt(doc.outstanding_amount)) < flt(doc.grand_total)) {
				if (doc.total_short_qty > 0 && !doc.short_qty){
	                                cur_frm.add_custom_button(__('Return / Debit Note for Short Qty'),
	...
	make_debit_note_for_shortqty: function() {
                frappe.model.open_mapped_doc({
                        method: "erpnext.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note_for_shortqty",
                        frm: cur_frm
                })
        },
	make_debit_note_for_rejected_qty: function() {
                frappe.model.open_mapped_doc({
                        method: "erpnext.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note_for_reject",
                        frm: cur_frm
                })
        },
});
