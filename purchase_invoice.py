...
def tally_integration():
	data_list=[]
	all_data=frappe.get_all("Purchase Invoice",filters={'docstatus':1},fields=['*'])
	for data in all_data:
		payload={}
		payload['purchase_invoice_no']=data.name
		payload['supplier']=data.supplier
		payload['supplier_tally_name']=data.supplier_tally_name
		if data.is_return==1:
			payload['debit_note']=True
			payload['net_total']=str(data.net_total).replace("-","")
		else:
			payload['debit_note']=False
			payload['net_total']=data.net_total
		payload['posting_date']=data.posting_date
		#payload['credit_account']=data.credit_to.split('-')[0].replace(' ','')
		payload['credit_account']=data.supplier_tally_name
		#payload['debit_account']=data.against_expense_account
		payload['tax_category']=data.tax_category
		#payload['taxes_and_charges']=data.total_taxes_and_charges
		doc=frappe.get_doc("Purchase Invoice",data.name)
		var_item=1
		for item in doc.items:
			payload[f'item{var_item}_expense_head']=item.expense_account
			var_item+=1
		var_taxes=1
		for row in doc.taxes:
			if row.tax_amount:
				account_head=row.account_head.split('-')[:2]
				if account_head[1]==" DFK":
					payload[f'gst_account_head{var_taxes}']=account_head[0]
				else:
					payload[f'gst_account_head{var_taxes}']=account_head[0]+"-"+account_head[1]
				if payload['debit_note']:
					payload[f'gst_account_head{var_taxes}_amount']=str(row.tax_amount).replace("-","")
				else:
					payload[f'gst_account_head{var_taxes}_amount']=row.tax_amount
				var_taxes+=1
		if payload['debit_note']:
			payload['total_gst_amount']=str(data.total_taxes_and_charges).replace("-","")
			payload['grand_total']=str(data.rounded_total).replace("-","")
			payload['round_off']=float(payload['grand_total'])-float(payload['total_gst_amount'])-float(payload['net_total'])
		else:
			payload['total_gst_amount']=data.total_taxes_and_charges
			payload['grand_total']=data.rounded_total
			payload['round_off']=float(payload['grand_total'])-float(payload['total_gst_amount'])-float(payload['net_total'])
		payload['narration_']=data.narration_
		payload['supplier_invoice']=data.bill_no
		payload['company']=data.company
		#payload['data']=data.taxes
		data_list.append(payload)
	return data_list

@frappe.whitelist()
def get_doc():
	data=frappe.get_doc("Purchase Invoice",'PI-DFK-00017')
	return data
